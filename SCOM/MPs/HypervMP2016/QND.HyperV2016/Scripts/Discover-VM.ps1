...
try
{
	...
	foreach($vm in $vms) {
		try {
			...
			$oInstance = $oDiscoveryData.CreateClassInstance("$MPElement[Name='QND.Hyperv.2016.VM']$")
			...
			$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/HostName$", $HostComputerIdentity)
			$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/VirtualMachineId$", $vm.Id.ToString())
			$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/VirtualMachineName$", $vm.VMName)

			if($vm.IntegrationServicesVersion) {$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/ISVersion$", $vm.IntegrationServicesVersion.ToString())}
			$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/RMEnabled$", (NullIsFalse $vm.ResourceMeteringEnabled))
			$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/VMGeneration$", (NullIsZero $vm.Generation))
			$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/HA$", (NullIsFalse $vm.IsClustered))
	#there's a bug in HypervPOSH for fixed memory VMs, sometimes wrong values are returned for minimum and maximum memory
			if($vm.DynamicMemoryEnabled) {
				$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/MinMemory$", (NullIsZero ($vm.MemoryMinimum/1MB)))
				$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/MaxMemory$", (NullIsZero ($vm.MemoryMaximum/1MB)))
				$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/StartMemory$", (NullIsZero ($vm.MemoryStartup/1MB)))
			}
			else {
				$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/MinMemory$", (NullIsZero ($vm.MemoryStartup/1MB)))
				$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/MaxMemory$", (NullIsZero ($vm.MemoryStartup/1MB)))
				$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/StartMemory$", (NullIsZero ($vm.MemoryStartup/1MB)))
			}
			if((NullIsFalse $vm.IsClustered) -eq $true) {$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/HVFarm$", $HVFarm)}
			else {$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/HVFarm$", $HostComputerIdentity)}

			if (![String]::IsNullOrEmpty($computerName)) {$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/VMComputerName$", $computerName)}
			if (![String]::IsNullOrEmpty($OSName))	{$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/OSName$", $OSName)}
			if (![String]::IsNullOrEmpty($OSVersion))	{$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/OSVersion$", $OSVersion)}

			if (![String]::IsNullOrEmpty($OSPlatformId))
			{
				$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/OSPlatformId$", [int]$OSPlatformId)
				If($OSPlatformId -eq 2) {$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/OSFamily$", 'Windows')}
				else {$oInstance.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/OSFamily$", 'Linux')}
			}
			...
			foreach($disk in $vm.HardDrives) {
				$oDisk = $oDiscoveryData.CreateClassInstance("$MPElement[Name='QND.Hyperv.2016.VirtualDrive']$")	
				$oDisk.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/VirtualMachineId$", $vm.Id.ToString())	
				...
				if($disk.ControllerType) {$oDisk.AddProperty("$MPElement[Name='QND.Hyperv.2016.VirtualDrive']/ConnectedControllerName$", $disk.ControllerType.ToString())}
				$oDisk.AddProperty("$MPElement[Name='QND.Hyperv.2016.VirtualDrive']/ImageFile$", $disk.Path)
				...
				If($details) {
					if($details.VhdFormat) {$oDisk.AddProperty("$MPElement[Name='QND.Hyperv.2016.VirtualDrive']/VHDFormat$", $details.VhdFormat.ToString())}
					if($details.VhdType) {$oDisk.AddProperty("$MPElement[Name='QND.Hyperv.2016.VirtualDrive']/VHDType$", $details.VhdType.ToString())}
					$oDisk.AddProperty("$MPElement[Name='QND.Hyperv.2016.VirtualDrive']/MaxSizeGB$", (NullIsZero ($details.Size/1GB)))
				}
				$oDiscoveryData.AddInstance($oDisk)
				#$rel = $oDiscoveryData.CreateRelationshipInstance("$MPElement[Name='QND.Hyperv.2016.VMHostsVMHardwareComponent']$")
			...
			foreach($nic in $vm.NetworkAdapters) {
				$oNic = $oDiscoveryData.CreateClassInstance("$MPElement[Name='QND.Hyperv.2016.VirtualNetworkAdapter']$")	
				$oNic.AddProperty("$MPElement[Name='QND.Hyperv.2016.VM']/VirtualMachineId$", $vm.Id.ToString())		
				...
				if($nic.SwitchId) {$oNic.AddProperty("$MPElement[Name='QND.Hyperv.2016.VirtualNetworkAdapter']/SwitchId$", $nic.SwitchId.ToString())}
				$oNic.AddProperty("$MPElement[Name='QND.Hyperv.2016.VirtualNetworkAdapter']/SwitchName$", $nic.SwitchName)
